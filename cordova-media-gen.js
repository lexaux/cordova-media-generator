...
function generate() {
    ...
    fs.readdir(path.join(process.cwd(), "platforms", "ios"), function (err, result) {
        ...
        if (!process.argv[2] && !config.image && (!config.icon && !config.splash)) {
            ...
            var imagesArray;
            var excludeImages = config.excludeImageSets;
            console.log(process.env.EXCLUDE_IMAGE_SETS);
            if (process.env.EXCLUDE_IMAGE_SETS) {
                excludeImages = process.env.EXCLUDE_IMAGE_SETS.split(',');
                console.log(JSON.stringify(excludeImages));
            }

            if (excludeImages) {
                console.log('Specifically excluding image sets:' + _(excludeImages).join(", "));
                var imageSetsToRetain = _(images).keys().difference(excludeImages).value();
                imagesArray = _(images).pick(imageSetsToRetain).values().flatten().value();
            ...
            var totalImages = imagesArray.length + (screenshots.pages.length * screenshots.screenshots.length);
